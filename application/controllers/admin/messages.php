...
class Messages_Controller extends Admin_Controller
{
    ...
    */
    function index($service_id = 5)
    {
        if( $service_id == 5 ) {
            $this->template->content = new View('admin/messages_feeds');
        } else {
        ...
        if ( $service_id == 5 ) { // Feeds
            $r_datepick = "";
            $filter_range = "";
            if( isset($_GET['datepick']) && !empty($_GET['datepick']) )
            {
                $r_datepick = $this->input->xss_clean($_GET['datepick']);
                $filter_range = "item_date between \"".date("Y-m-d",strtotime($r_datepick))." 00:00:00\" and \"".date("Y-m-d",strtotime($r_datepick))." 23:59:00\"";
            }
            $feed_id = "";
            if ( isset($_GET['feed_id'])  AND !empty($_GET['feed_id']) )
            {
                $feed_id = $this->input->xss_clean($_GET['feed_id']);
            ...
            if ( $_POST ) {
                ...
                if( $post->validate() )
                {
                    ...
                    {
                        foreach($post->message_id as $item_id)
                        {
                            // Delete Message
            ...
            $this->template->content->datepick = $r_datepick;
            $this->template->content->sel_feeds = $feed_id;
