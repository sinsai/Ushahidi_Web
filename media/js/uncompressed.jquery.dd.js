   ...
   var dd = function(element, options)
   {
	...
	var createATags = function () {
		...
		childnodes.each(function(current){
								if(childcount % options.visibleRows==1){
									aTag += "<div style=\"float:left;\">";
									colcount++;
								}
								...
								if(childcount % options.visibleRows==0){
									aTag += "</div>";
								}
								 childcount++;
								 });
	...
	var createDropDown = function () {
		...
		$("#"+childid).css("width", (iWidth-2)*colcount+"px");
		if(allOptions.length>options.visibleRows) {
			var margin = parseInt($("#"+childid+" a:first").css("padding-bottom")) + parseInt($("#"+childid+" a:first").css("padding-top"));
			var iHeight = ((options.rowHeight)*(options.visibleRows-1)) - margin;
			$("#"+childid).css("height", (iHeight+7)+"px");
		} else if(ddList) {
			var iHeight = $("#"+elementid).height();
